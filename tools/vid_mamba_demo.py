...
from val_to_imdb_mamba import Predictor
...
def make_parser():
    parser = argparse.ArgumentParser("MCTMdet Demo!")
    parser.add_argument("-expn", "--experiment-name", type=str, default=None)
    parser.add_argument("-n", "--name", type=str, default=None, help="model name")

    parser.add_argument(
        "--path", default="/mnt/yingpan/jgdong/datasets/ILSVRC2015/Data/VID/snippets/val/ILSVRC2015_val_00007027.mp4", help="path to images or video"
    )
    ...
    parser.add_argument(
        "-f",
        "--exp_file",
        default='./exps/MCTMDet/mamba_memory_use.py',
        type=str,
        help="pls input your expriment description file",
    )
    parser.add_argument("-c", "--ckpt", default='mamba_memory_unuse_best_ckpt.pth', type=str, help="ckpt for eval")
    parser.add_argument(
        "--device",
        default="gpu",
        type=str,
    ...
    parser.add_argument('--lframe', default=4, help='local frame num')
    parser.add_argument('-b', '--batch_size', default=8, type=int, help='Batch size for evaluation')
...
def imageflow_demo(predictor, vis_folder, current_time, args,exp):
    ...
    current_batch = []
    for clip in res:
        current_batch.append(torch.stack(clip))
        if len(current_batch) == batch_size:
            batch_tensor = torch.stack(current_batch)  # B×L×C×H×W
            res_batches.append(batch_tensor)
            current_batch = []
    if current_batch:
    ...
    for batch in res_batches:
        if batch==[]: continue
        outputs.extend(predictor.inference(batch))
