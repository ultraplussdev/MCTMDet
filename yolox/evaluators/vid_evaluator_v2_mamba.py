...
class VIDEvaluator:
    ...
    def evaluate(
        ...
        for cur_iter, batch in enumerate(progress_bar(self.dataloader)):
            ...
            label = batch["targets_list"]
            batch_size = len(label)  # 2
            L = len(label[0])  # 3
            label = [label[b][l] for b in range(batch_size) for l in range(L)]
            label = [t[t.any(dim=1)] for t in label]
            path = batch['frame_paths']
            info_imgs = batch['img_infos']
            info_imgs=[ info_imgs[b][l] for b in range(batch_size) for l in range(L)]
            with torch.no_grad():
                ...
                with autocast(enabled=half):
                    outputs = model(imgs, targets=label)
                outputs = postprocess(
